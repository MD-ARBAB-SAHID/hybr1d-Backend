...
const addProducts = async (req,res,next)=>{
    ...
    if(!existingSeller)
    return next(new HttpError("Could not add products to catalog,try again",500));
...
//saving product and adding product id to the seller's catalog
    try{
       ...
       }catch(err)
       {
        ...
        return next(new HttpError("Could not add product to catalog,try again",500));
       }
...
const getOrders = async(req,res,next)=>{
    ...
    let orders;
    try{
        orders = await Order.find({seller:userId}).populate({path:"buyer" , select:"name email"}).populate({path:"products",select:"name price"});    
    }catch(err)
    {
